...
<template>
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
      <div>
        <h1 class="text-[15px] font-bold text-gray-900">SEO — Tüm Sayfalar</h1>
        <p class="text-xs text-gray-400 mt-0.5">
          Sitenin tüm sayfalarının SEO durumunu tek ekrandan yönet.
        </p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <button class="hdr-btn-outlined" @click="router.push('/seo/404s')">404 Logları</button>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
      <input
        v-model="search"
        type="text"
        :placeholder="`${selectedDoctype.label} içinde ara...`"
      ...
      >
        ...
        <option value="missing_meta">Meta başlık boş</option>
        <option value="missing_desc">Meta açıklama boş</option>
        <option value="noindex">Noindex işaretli</option>
      </select>
    </div>

    <!-- Loading / Empty -->
    <div v-if="loading" class="text-center py-12 text-sm text-gray-400">Yükleniyor...</div>
    <div
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-xs uppercase text-gray-500">
          <tr>
            ...
            <th class="px-4 py-3 text-left font-semibold">Meta Başlık</th>
            <th class="px-4 py-3 text-center font-semibold">Meta Açk.</th>
            <th class="px-4 py-3 text-center font-semibold">Noindex</th>
            <th class="px-4 py-3 text-right font-semibold">Aksiyon</th>
          </tr>
        </thead>
