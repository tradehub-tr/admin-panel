...
async function loadDoc() {
  ...
  try {
    ...
    if (isNew.value) {
      // Pre-fill from URL query — any query param matching a DocType fieldname
      // becomes its default value on the new form.
      const validFields = new Set((metaFields.value || []).map(f => f.fieldname))
      const queryDefaults = {}
      for (const [key, val] of Object.entries(route.query || {})) {
        if (key === 'returnTo') continue
        if (validFields.has(key)) queryDefaults[key] = val
      }
      formData.value = { ...emptyFromMeta, ...queryDefaults }
    } else {
      try {
