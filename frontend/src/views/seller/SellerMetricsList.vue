  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="hdr-btn-outlined" data-tour="smt-refresh" @click="loadData()">
    ...
    <!-- Search & Sort -->
    <div class="card mb-5 !p-3" data-tour="smt-toolbar">
    ...
    <!-- Table -->
    <div v-else class="card p-0 overflow-hidden" data-tour="smt-table">
            ...
            >
              ...
              <td class="tbl-td text-center">
                <span
                  ...
                  :style="{ color: getRateColor(item.on_time_delivery_rate, true) }"
                  >%{{ (item.on_time_delivery_rate || 0).toFixed(1) }}</span
                >
              </td>
              <td class="tbl-td text-center">
                <span
                  class="text-xs font-semibold"
                  :style="{ color: getRateColor(item.cancellation_rate, false) }"
                  >%{{ (item.cancellation_rate || 0).toFixed(1) }}</span
                >
              </td>
              <td class="tbl-td text-center">
                <span
                  class="text-xs font-semibold"
                  :style="{ color: getRateColor(item.return_rate, false) }"
                  >%{{ (item.return_rate || 0).toFixed(1) }}</span
                >
              </td>
              <td class="tbl-td">
      ...
      <div v-else-if="viewMode === 'grid'" class="list-grid">
        ...
        >
          <div class="flex items-center gap-2.5 mb-3">
            <div
              class="w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold text-white"
              :class="getColor(item.seller)"
            >
              {{ (item.seller || "?")[0] }}
            </div>
...
<script setup>
  ...
  usePageTour("seller-metrics-list", () => [
    {
      target: '[data-tour="smt-toolbar"]',
      title: t("tourSteps.page.smtToolbar_t"),
      desc: t("tourSteps.page.smtToolbar_d"),
    },
    {
      target: '[data-tour="smt-table"]',
      title: t("tourSteps.page.smtTable_t"),
      desc: t("tourSteps.page.smtTable_d"),
    },
    {
      target: '[data-tour="smt-refresh"]',
      title: t("tourSteps.page.smtRefresh_t"),
      desc: t("tourSteps.page.smtRefresh_d"),
    },
  ]);
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await api.getList("Seller Metrics", {
        fields: listFields,
        filters,
  ...
  function getColor(s) {
    ...
    return c[(s || "").charCodeAt((s || "").length - 1 || 0) % c.length];
  }
  function getRateColor(v, higherBetter) {
