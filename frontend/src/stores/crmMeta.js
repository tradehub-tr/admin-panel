...
export const useCrmMetaStore = defineStore('crmMeta', () => {
  ...
  async function loadCommunicationStatuses() {
    ...
    } catch { communicationStatuses.value = [] }
  }
  ...
  return {
    users, organizations,
    leadStatuses, dealStatuses, leadSources, industries, territories, lostReasons, communicationStatuses,
    sessionFlags, loaded,
    loadAll, loadLeadStatuses, loadDealStatuses, loadLeadSources, loadIndustries, loadTerritories,
    loadLostReasons, loadCommunicationStatuses, loadUsers, loadOrganizations, loadSessionFlags,
    getFilterableFields, getQuickFilters, getSortOptions,
    userByEmail, invalidate,
  }
})
