      ...
      >
        ...
        >
          <div class="flex items-start gap-3 mb-4">
            ...
            <div class="flex-1">
              <h3 :id="titleId" class="text-lg font-bold text-gray-900 dark:text-gray-100">
                {{ titleLabel }}
              </h3>
...
<script setup>
  ...
  // ── Dialog erişilebilirliği (WCAG 2.1.2 / 2.4.3 / 4.1.2) ──
  // Açılışta odak panele girer, Tab panel içinde döner, kapanışta odak
  // dialogu açan tetikleyiciye geri verilir.
  const panelRef = ref(null);
  const titleId = useId();
  let lastActive = null;

  // Seçici dizesi ve tuzak mantığı ortak modülde (`common/focusTrap.js`) —
  // ResolveDialog vb. kopyaların ayrışmaması için tek kaynak.
  function trapTab(e) {
