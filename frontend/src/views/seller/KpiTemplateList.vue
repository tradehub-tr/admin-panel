    ...
    <div v-else class="card p-0 overflow-hidden">
        ...
        <table class="w-full">
          ...
          <tbody>
            ...
            >
              ...
              <td class="tbl-td text-center">
                <span class="text-xs font-medium text-gray-400">{{
                  getFreqLabel(item.evaluation_frequency)
                }}</span>
              </td>
              <td class="tbl-td text-center">
      ...
      <div v-else-if="viewMode === 'list'">
        ...
        >
          ...
          <span class="tpl-status-badge" :class="getTplStatusCls(item.status)"
            ><span class="tpl-dot"></span>{{ getTplStatusLabel(item.status) }}</span
          >
      ...
      <div v-else-if="viewMode === 'grid'" class="list-grid">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="tpl-status-badge text-[10px]" :class="getTplStatusCls(item.status)"
              ><span class="tpl-dot"></span>{{ getTplStatusLabel(item.status) }}</span
            >
          </div>
...
<script setup>
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await api.getList("KPI Template", {
        fields: listFields,
        filters,
