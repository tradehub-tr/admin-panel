...
export class PolicyEngine {
	...
	private code(policy: Dict, rule: string): string {
		// F-13b — video standartları dışa açılacak kodları
		// `video.validation_codes` içinde İLAN EDER; kural adından türetilen
		// `${prefix}_${rule}` bunlarla örtüşmüyor. Python ikizi (`engine.py::
		// _code`) bu aramayı yapıyordu, TS tarafı ATLIYORDU: aynı videoya
		// backend `cover_video_too_long`, panel `cover_video_duration_too_long`
		// diyordu. Kod dış API sözleşmesi olduğu için ayrışma sessiz kalmıyor —
		// panelin gösterdiği kod hiçbir belgede karşılık bulmuyordu.
