  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-[15px] font-bold text-gray-900 dark:text-gray-100 truncate">
          {{ doc.kpi_type || t("sellerKpiDetail.title") }}
        </h1>
      </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
        <div class="card !p-4 text-center">
          ...
          <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">
            {{ t("sellerKpiDetail.success") }}
          </p>
        </div>
        <div class="card !p-4 text-center">
          <p class="text-2xl font-black text-blue-600">{{ formatVal(doc.target_value) }}</p>
          <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">
            {{ t("sellerKpiDetail.target") }}
          </p>
        </div>
        <div class="card !p-4 text-center">
          <p class="text-2xl font-black text-emerald-600">{{ formatVal(doc.actual_value) }}</p>
          <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">
            {{ t("sellerKpiDetail.actual") }}
          </p>
        </div>
        <div class="card !p-4 text-center">
          <p class="text-2xl font-black" :class="getTrendColor(doc.value_trend)">
            {{ getTrendLabel(doc.value_trend) }}
          </p>
          <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">
            {{ t("sellerKpiDetail.trend") }}
          </p>
        </div>
        <div class="card !p-4 text-center">
      ...
      <div v-if="activeTab === 'details'">
        <div class="card mb-5">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.seller") }} *</label>
              <input :value="doc.seller" type="text" class="form-input" readonly />
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.kpiCode") }}</label>
              <input :value="doc.name" type="text" class="form-input" readonly />
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.kpiType") }}</label>
              <input :value="doc.kpi_type" type="text" class="form-input" readonly />
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.kpiCategory") }}</label>
              <input
                :value="getCategoryLabel(doc.kpi_category)"
                type="text"
                class="form-input"
                readonly
              />
        ...
        <div class="card mb-5">
          <h3 class="section-title">
            ...
            >{{ t("sellerKpiDetail.periodInfo") }}
          </h3>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.periodType") }}</label>
              <input :value="doc.period_type" type="text" class="form-input" readonly />
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.periodLabel") }}</label>
              <input
                :value="doc.period_label || doc.evaluation_period || '-'"
                type="text"
                class="form-input"
                readonly
              />
            ...
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.start") }}</label>
              <input
                :value="formatDate(doc.period_start)"
                type="text"
                class="form-input"
                readonly
              />
            ...
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.end") }}</label>
        ...
        <div class="card">
          <h3 class="section-title">
            <i class="fas fa-clock text-amber-500 mr-2"></i>{{ t("sellerKpiDetail.statusTime") }}
          </h3>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.status") }}</label>
              <div class="mt-1">
                <span class="kpi-status-badge" :class="getKpiStatusCls(doc.status)">
                  <span class="kst-dot"></span>
                  {{ getKpiStatusLabel(doc.status) }}
            ...
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.lastCalculation") }}</label>
              <input
                :value="formatDateTime(doc.last_calculated_at)"
                type="text"
                class="form-input"
                readonly
              />
            ...
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.createdBy") }}</label>
              <input :value="doc.created_by || doc.owner" type="text" class="form-input" readonly />
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.calculationMethod") }}</label>
              <input
                :value="doc.calculation_method || '-'"
                type="text"
                class="form-input"
                readonly
              />
      ...
      <div v-if="activeTab === 'targets'">
        <div class="card mb-5">
          <h3 class="section-title">
            ...
            >{{ t("sellerKpiDetail.targetVsActual") }}
          </h3>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            <div v-for="m in targetMetrics" :key="m.field" class="score-card">
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-sm font-bold" :style="{ color: m.color || '#6366f1' }">{{
                  formatVal(doc[m.field])
                }}</span>
              </div>
              <div v-if="m.max" class="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  :style="{
                    width: getBarPercent(doc[m.field], m.max) + '%',
        ...
        <div class="card">
          <h3 class="section-title">
            <i class="fas fa-sliders text-amber-500 mr-2"></i>{{ t("sellerKpiDetail.thresholds") }}
          </h3>
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
            <div class="text-center p-3 bg-emerald-50 rounded-xl">
              <p class="text-xl font-black text-emerald-600">
                {{ formatVal(doc.minimum_threshold) }}
              </p>
              <p class="text-[10px] text-gray-500 mt-1">{{ t("sellerKpiDetail.minThreshold") }}</p>
            </div>
            <div class="text-center p-3 bg-amber-50 rounded-xl">
              <p class="text-xl font-black text-amber-600">
                {{ formatVal(doc.warning_threshold) }}
              </p>
              <p class="text-[10px] text-gray-500 mt-1">
                {{ t("sellerKpiDetail.warningThreshold") }}
              </p>
            </div>
            <div class="text-center p-3 bg-red-50 rounded-xl">
              <p class="text-xl font-black text-red-600">{{ formatVal(doc.critical_threshold) }}</p>
              <p class="text-[10px] text-gray-500 mt-1">
                {{ t("sellerKpiDetail.criticalThreshold") }}
              </p>
            </div>
            <div class="text-center p-3 bg-blue-50 rounded-xl">
              <p class="text-xl font-black text-blue-600">{{ formatVal(doc.maximum_threshold) }}</p>
              <p class="text-[10px] text-gray-500 mt-1">{{ t("sellerKpiDetail.maxThreshold") }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'benchmark'">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            ...
            <div class="perf-card">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-xs font-semibold text-gray-600">{{
                  t("sellerKpiDetail.categoryAverage")
                }}</span>
              </div>
            ...
            <div class="perf-card">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-xs font-semibold text-gray-600">{{
                  t("sellerKpiDetail.platformAverage")
                }}</span>
              </div>
        ...
        <div class="card">
          <h3 class="section-title">
            ...
            >{{ t("sellerKpiDetail.statistics") }}
          </h3>
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
            <div class="text-center p-3 bg-gray-50 rounded-xl">
              <p class="text-xl font-black text-gray-800">{{ doc.sample_size || 0 }}</p>
              <p class="text-[10px] text-gray-400 mt-1">{{ t("sellerKpiDetail.sample") }}</p>
            </div>
            <div class="text-center p-3 bg-gray-50 rounded-xl">
              <p class="text-xl font-black text-gray-800">{{ doc.data_points_count || 0 }}</p>
              <p class="text-[10px] text-gray-400 mt-1">{{ t("sellerKpiDetail.dataPoints") }}</p>
            </div>
            <div class="text-center p-3 bg-gray-50 rounded-xl">
              <p class="text-xl font-black text-gray-800">
                %{{ formatScore(doc.confidence_level) }}
              </p>
              <p class="text-[10px] text-gray-400 mt-1">
                {{ t("sellerKpiDetail.confidenceLevel") }}
              </p>
            </div>
            <div class="text-center p-3 bg-gray-50 rounded-xl">
              <p class="text-xl font-black text-gray-800">
                {{ formatVal(doc.standard_deviation) }}
              </p>
              <p class="text-[10px] text-gray-400 mt-1">{{ t("sellerKpiDetail.stdDeviation") }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'actions'">
        <div class="card mb-5">
          <h3 class="section-title">
            <i class="fas fa-bell text-red-500 mr-2"></i>{{ t("sellerKpiDetail.alerts") }}
          </h3>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.alertsEnabled") }}</p>
                  <p
                    class="text-sm font-bold mt-1"
                    :class="doc.alerts_enabled ? 'text-emerald-600' : 'text-gray-400'"
                  >
                    {{ doc.alerts_enabled ? t("sellerKpiDetail.yes") : t("sellerKpiDetail.no") }}
                  </p>
                </div>
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.alertCount") }}</p>
                  <p class="text-sm font-bold text-gray-800 mt-1">{{ doc.alert_count || 0 }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.lastAlert") }}</p>
                  <p class="text-sm font-bold text-gray-800 mt-1">
                    {{ formatDateTime(doc.last_alert_sent) }}
                  </p>
                </div>
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.daysBelowTarget") }}</p>
                  <p class="text-sm font-bold text-red-600 mt-1">
                    {{ doc.days_below_target || 0 }}
                  </p>
                </div>
              </div>
            </div>
            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.daysAtRisk") }}</p>
                  <p class="text-sm font-bold text-amber-600 mt-1">{{ doc.days_at_risk || 0 }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-500">
                    {{ t("sellerKpiDetail.consecutivePeriodsBelow") }}
                  </p>
                  <p class="text-sm font-bold text-red-600 mt-1">
                    {{ doc.consecutive_periods_below || 0 }}
                  </p>
                </div>
                <div>
                ...
                <div>
                  <p class="text-xs text-gray-500">{{ t("sellerKpiDetail.deviation") }}</p>
                  <p class="text-sm font-bold text-gray-800 mt-1">
                    {{ formatVal(doc.deviation_from_target) }}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="card mb-5">
          <h3 class="section-title">
            ...
            >{{ t("sellerKpiDetail.action") }}
          </h3>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.actionRequired") }}</label>
              <div class="mt-1">
                <span
                  class="text-sm font-bold"
                  :class="doc.action_required ? 'text-red-600' : 'text-emerald-600'"
                  >{{
                    doc.action_required ? t("sellerKpiDetail.yes") : t("sellerKpiDetail.no")
                  }}</span
                >
              </div>
            </div>
            <div>
              <label class="form-label">{{ t("sellerKpiDetail.deadline") }}</label>
              <input
                :value="formatDate(doc.action_deadline)"
                type="text"
                class="form-input"
                readonly
              />
            ...
            <div class="lg:col-span-2">
              <label class="form-label">{{ t("sellerKpiDetail.recommendedAction") }}</label>
              <div class="p-3 bg-gray-50 rounded-lg min-h-[48px]">
                <p class="text-xs text-gray-600">{{ doc.recommended_action || "-" }}</p>
              </div>
            </div>
            <div class="lg:col-span-2">
              <label class="form-label">{{ t("sellerKpiDetail.actionTaken") }}</label>
              <div class="p-3 bg-gray-50 rounded-lg min-h-[48px]">
        ...
        <div class="card">
          <h3 class="section-title">
            <i class="fas fa-note-sticky text-gray-500 mr-2"></i>{{ t("sellerKpiDetail.notes") }}
          </h3>
          <div class="mt-3 p-3 bg-gray-50 rounded-lg min-h-[80px]">
            <p class="text-xs text-gray-600 whitespace-pre-wrap">
              {{ doc.notes || t("sellerKpiDetail.noNotes") }}
            </p>
          </div>
        </div>
      </div>
...
<script setup>
  ...
  const tabs = computed(() => [
    { key: "details", label: t("sellerKpiDetail.tabDetails"), icon: "fas fa-file-lines" },
    { key: "targets", label: t("sellerKpiDetail.tabTargets"), icon: "fas fa-bullseye" },
    { key: "benchmark", label: t("sellerKpiDetail.tabBenchmark"), icon: "fas fa-ranking-star" },
    { key: "actions", label: t("sellerKpiDetail.tabActions"), icon: "fas fa-clipboard-check" },
  ]);

  const workflowSteps = computed(() => [
    { value: "Draft", label: t("sellerKpiDetail.stepDraft") },
    { value: "Active", label: t("sellerKpiDetail.stepActive") },
    { value: "On Track", label: t("sellerKpiDetail.stepOnTrack") },
    { value: "Exceeding", label: t("sellerKpiDetail.stepExceeding") },
    { value: "Calculated", label: t("sellerKpiDetail.stepCalculated") },
  ]);

  const currentStepIndex = computed(() => {
    const status = doc.value.status;
    const idx = workflowSteps.value.findIndex((s) => s.value === status);
    return idx >= 0 ? idx : -1;
  });

  const targetMetrics = computed(() => [
    { field: "target_value", label: t("sellerKpiDetail.targetValue"), color: "#3b82f6" },
    {
      field: "actual_value",
      label: t("sellerKpiDetail.actualValue"),
      color: "#10b981",
      max: "target_value",
    },
    { field: "previous_value", label: t("sellerKpiDetail.previousValue"), color: "#8b5cf6" },
    { field: "value_change", label: t("sellerKpiDetail.valueChange"), color: "#f59e0b" },
    {
      field: "improvement_target",
      label: t("sellerKpiDetail.improvementTarget"),
      color: "#ec4899",
    },
    {
      field: "score_contribution",
      label: t("sellerKpiDetail.scoreContributionMetric"),
      color: "#6366f1",
    },
  ]);

  function isStepDone(i) {
    return i < currentStepIndex.value;
  }
  ...
  function getTrendLabel(trend) {
    const m = {
      Improving: t("sellerKpiDetail.trendImproving"),
      Stable: t("sellerKpiDetail.trendStable"),
      Declining: t("sellerKpiDetail.trendDeclining"),
      Volatile: t("sellerKpiDetail.trendVolatile"),
      New: t("sellerKpiDetail.trendNew"),
    };
  ...
  function getCategoryLabel(cat) {
    const m = {
      Fulfillment: t("sellerKpiDetail.catFulfillment"),
      Delivery: t("sellerKpiDetail.catDelivery"),
      Quality: t("sellerKpiDetail.catQuality"),
      Service: t("sellerKpiDetail.catService"),
      Compliance: t("sellerKpiDetail.catCompliance"),
      Engagement: t("sellerKpiDetail.catEngagement"),
      Financial: t("sellerKpiDetail.catFinancial"),
    };
    return m[cat] || cat || "-";
  }
  ...
  function getKpiStatusLabel(st) {
    const m = {
      Draft: t("sellerKpiDetail.stDraft"),
      Active: t("sellerKpiDetail.stActive"),
      "On Track": t("sellerKpiDetail.stOnTrack"),
      "At Risk": t("sellerKpiDetail.stAtRisk"),
      "Below Target": t("sellerKpiDetail.stBelowTarget"),
      Critical: t("sellerKpiDetail.stCritical"),
      Exceeding: t("sellerKpiDetail.stExceeding"),
      Paused: t("sellerKpiDetail.stPaused"),
      Expired: t("sellerKpiDetail.stExpired"),
      Calculated: t("sellerKpiDetail.stCalculated"),
    };
    return m[st] || st || "-";
  }
