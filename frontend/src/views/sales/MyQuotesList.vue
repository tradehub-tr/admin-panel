  ...
  <div>
    ...
    <div class="flex gap-1.5 mb-4 flex-wrap">
      <button v-for="f in statusFilters" :key="f.value"
        ...
        <span class="inline-block w-1.5 h-1.5 rounded-full mr-1.5" :class="f.dot"></span>{{ f.label }}
      </button>
    </div>
    ...
    <div v-else class="space-y-2">
      <div v-for="q in items" :key="q.name"
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[13px] font-semibold text-gray-200 mt-1 truncate">{{ q.rfq_product_name || '-' }}</p>
        ...
        <div class="text-right">
          ...
          <p v-if="q.lead_time_days" class="text-[10px] text-gray-500">{{ q.lead_time_days }} gün</p>
        </div>
...
function getQuoteStatusCls(s) {
  return { Submitted: 'qs-submitted', Accepted: 'qs-accepted', Rejected: 'qs-rejected', Withdrawn: 'qs-withdrawn' }[s] || 'qs-submitted'
}
function getQuoteStatusLabel(s) {
  return { Submitted: 'Gönderildi', Accepted: 'Kabul Edildi', Rejected: 'Reddedildi', Withdrawn: 'Geri Çekildi' }[s] || s || '-'
}
...
<style scoped>
.quote-status-badge { display: inline-flex; align-items: center; font-size: 11px; font-weight: 600; padding: 5px 12px; border-radius: 6px; white-space: nowrap; }
.quote-status-badge .rfq-dot { width: 7px; height: 7px; border-radius: 50%; margin-right: 6px; flex-shrink: 0; }
.qs-submitted { background: rgba(245, 158, 11, 0.15); color: #f59e0b; }
.qs-submitted .rfq-dot { background: #f59e0b; }
.qs-accepted { background: rgba(16, 185, 129, 0.15); color: #10b981; }
.qs-accepted .rfq-dot { background: #10b981; }
.qs-rejected { background: rgba(239, 68, 68, 0.15); color: #ef4444; }
.qs-rejected .rfq-dot { background: #ef4444; }
.qs-withdrawn { background: rgba(107, 114, 128, 0.15); color: #6b7280; }
.qs-withdrawn .rfq-dot { background: #6b7280; }
</style>
