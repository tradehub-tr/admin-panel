        ...
        <table class="w-full">
          ...
          <tbody>
            ...
            >
              ...
              <td class="tbl-td">
                <span
                  class="score-type-badge"
                  :class="'type-' + (item.score_type || '').toLowerCase()"
                >
              ...
              <td class="tbl-td">
                <span
                  class="status-badge"
                  :class="'st-' + (item.status || '').toLowerCase().replace(/\s+/g, '-')"
                >
      ...
      <div v-else-if="viewMode === 'list'">
        ...
        >
          ...
          <span
            class="status-badge"
            :class="'st-' + (item.status || '').toLowerCase().replace(/\s+/g, '-')"
            ><span class="st-dot"></span>{{ getStatusLabel(item.status) }}</span
          >
          <span
            class="text-sm font-bold flex-shrink-0"
            :style="{ color: getScoreColor(item.overall_score) }"
            >{{ formatScore(item.overall_score) }}</span
          >
      ...
      <div v-else-if="viewMode === 'grid'" class="list-grid">
        ...
        >
          ...
          <div class="text-center mb-2">
            <span class="text-lg font-bold" :style="{ color: getScoreColor(item.overall_score) }">{{
              formatScore(item.overall_score)
            }}</span>
          </div>
          ...
          <div class="kanban-col-body">
            ...
            >
              ...
              <div class="kanban-card-meta">
                <span class="font-bold" :style="{ color: getScoreColor(item.overall_score) }">{{
                  formatScore(item.overall_score)
                }}</span>
...
<style scoped>
  ...
  .type-weekly {
    background: var(--th-type-weekly-bg);
    color: var(--th-type-weekly-fg);
  }
  .type-monthly {
    background: var(--th-type-monthly-bg);
    color: var(--th-type-monthly-fg);
  }
  .type-annual {
    background: var(--th-type-annual-bg);
    color: var(--th-type-annual-fg);
  }
  .type-daily {
    background: var(--th-type-daily-bg);
    color: var(--th-type-daily-fg);
  }
  .type-periodic {
    background: var(--th-type-periodic-bg);
    color: var(--th-type-periodic-fg);
  }
  .type-lifetime {
    background: var(--th-type-lifetime-bg);
    color: var(--th-type-lifetime-fg);
  }
  .type-manual {
    background: var(--th-type-manual-bg);
    color: var(--th-type-manual-fg);
  }
  ...
  .st-draft {
    background: var(--th-st-draft-bg);
    color: var(--th-st-draft-fg);
  }
  .st-draft .st-dot {
    background: var(--th-st-draft-dot);
  }
  .st-calculating {
    background: var(--th-st-calculating-bg);
    color: var(--th-st-calculating-fg);
  }
  .st-calculating .st-dot {
    background: var(--th-st-calculating-dot);
  }
  .st-pending-review {
    background: var(--th-st-pending-review-bg);
    color: var(--th-st-pending-review-fg);
  }
  .st-pending-review .st-dot {
    background: var(--th-st-pending-review-dot);
  }
  .st-finalized {
    background: var(--th-st-finalized-bg);
    color: var(--th-st-finalized-fg);
  }
  .st-finalized .st-dot {
    background: var(--th-st-finalized-dot);
  }
  .st-revised {
    background: var(--th-st-revised-bg);
    color: var(--th-st-revised-fg);
  }
  .st-revised .st-dot {
    background: var(--th-st-revised-dot);
  }
  .st-appealed {
    background: var(--th-st-appealed-bg);
    color: var(--th-st-appealed-fg);
  }
  .st-appealed .st-dot {
