...
test("get_packing_queue — sözleşme §2.1 şekli", async () => {
  ...
  assert.deepEqual(
    Object.keys(res.buckets)
      .sort()
      .filter((k) => !["unpacked", "partial", "awaiting_label", "ready"].includes(k)),
    [],
...
test("MOCK haritası sözleşmedeki 11 ucun HEPSİNİ kapsıyor", () => {
  ...
  const anahtarlar = [...blok.slice(0, blok.indexOf("};")).matchAll(/^\s{2}(\w+):/gm)].map(
    (m) => m[1]
  );
