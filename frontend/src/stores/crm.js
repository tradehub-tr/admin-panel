...
const LEAD_FIELDS = [
  ...
  'mobile_no', 'organization', 'status', 'source', 'industry',
  'territory', 'job_title', 'lead_owner', 'modified', 'creation',
  'no_of_employees', 'annual_revenue',
]

const DEAL_FIELDS = [
  'name', 'organization', 'deal_owner', 'status',
  'close_date', 'annual_revenue', 'probability', 'currency',
  'deal_value', 'expected_deal_value', 'expected_closure_date',
  'industry', 'territory', 'no_of_employees',
  'modified', 'creation',
]
...
export const useCrmStore = defineStore('crm', () => {
  ...
  // ── CRM Lead ─────────────────────────────────────────
  async function fetchLeads({ page = 1, pageSize = 20, filters = [], orFilters, orderBy = 'modified desc' } = {}) {
  ...
  // ── CRM Deal ─────────────────────────────────────────
  async function fetchDeals({ page = 1, pageSize = 20, filters = [], orFilters, orderBy = 'modified desc' } = {}) {
  ...
  return {
    ...
    fetchDeals, fetchAllDealsForKanban, fetchDeal, createDeal, updateDeal, updateDealStatus,
  }
})
