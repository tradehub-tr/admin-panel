            ...
            <td class="px-4 py-3">
              <div class="space-y-0.5">
                <p
                  ...
                  class="text-xs text-gray-600 dark:text-gray-400 truncate max-w-[200px]"
                >
          ...
          <div class="flex items-start justify-between mb-5">
            ...
            <span
              ...
              :class="{
                ...
                'bg-red-50 text-red-700 border border-red-200':
                  refundDetailOrder?.refund_status === 'Rejected',
              }"
            >
              <AppIcon name="alert-circle" :size="10" />
              {{
          ...
          <div class="flex gap-2.5 justify-end">
            ...
            <template v-if="refundDetailOrder?.refund_status === 'Pending'">
              <button
                ...
                @click="
                  handleRefund(refundDetailOrder, 'reject');
              ...
              <button
                ...
                @click="
                  handleRefund(refundDetailOrder, 'approve');
                  showRefundModal = false;
                "
              >
...
<script setup>
  ...
  async function fetchBadgeCounts() {
    try {
      const [unpaidRes, refundRes] = await Promise.all([
        api.callMethod("tradehub_core.api.order.get_seller_orders", {
          status: "unpaid",
          page: 1,
          page_size: 1,
        }),
  ...
  async function doShipOrder() {
    ...
    try {
      await api.callMethod(
        "tradehub_core.api.order.seller_ship_order",
        {
  ...
  async function handleRefund(order, action) {
    try {
      await api.callMethod(
        "tradehub_core.api.order.seller_handle_refund",
        {
