...
export const useCrmContactStore = defineStore('crmContacts', () => {
  ...
  async function fetchContacts({ page = 1, pageSize = 30, filters = [], orFilters, orderBy = 'modified desc' } = {}) {
    loading.value = true
    try {
      const params = {
  ...
  async function getLinkedDeals(contactName) {
    try {
      const res = await api.callMethod('crm.api.contact.get_linked_deals', { contact: contactName })
      return res.message || []
    } catch { return [] }
  }
  ...
  async function setAsPrimary(contactName, field /* 'email' | 'mobile' */, value) {
    try {
      return await api.callMethod('crm.api.contact.set_as_primary', {
        contact: contactName, field, value,
      })
    } catch (e) {
      throw e
    }
  ...
  return {
    ...
    getLinkedDeals, setAsPrimary, searchEmails,
  }
})
