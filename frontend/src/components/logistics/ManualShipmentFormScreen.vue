<template>
  <form class="space-y-6" @submit.prevent="$emit('save', draft)">
    <div class="flex flex-wrap items-center gap-3">
      <div>
        <h1 class="text-lg font-semibold">{{ t("logistics.manual.title") }}</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400">{{ t("logistics.manual.subtitle") }}</p>
      </div>
      <div class="ms-auto flex gap-2">
        <button type="button" class="th-btn-outline text-sm" @click="$emit('cancel')">
          {{ t("logistics.form.cancel") }}
        </button>
        <button type="submit" class="th-btn-primary text-sm" :disabled="saving || !isValid">
    ...
    <section class="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700">
      <h2 class="text-sm font-semibold">{{ t("logistics.manual.carrierSection") }}</h2>
      <div class="grid gap-4 sm:grid-cols-2">
        <label class="block">
          <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.order") }} *</span>
          <LinkInput v-model="draft.order" doctype="Order" />
        </label>

        <label class="block">
          <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.channel") }} *</span>
        ...
        <template v-if="needsCarrier">
          ...
          <label class="block">
            <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.service") }}</span>
            <LinkInput v-model="draft.carrier_service" doctype="Carrier Service" />
          </label>
          <label class="block">
            <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.trackingNumber") }}</span>
            <input v-model="draft.tracking_number" type="text" class="form-input" />
            <span class="mt-1 block text-xs text-slate-500">{{ t("logistics.manual.trackingHint") }}</span>
          </label>
        </template>

        <template v-else>
          <label class="block">
            <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.driverName") }}</span>
            <input v-model="draft.driver_name" type="text" class="form-input" />
          </label>
          <label class="block">
            <span class="mb-1 block text-sm font-medium">{{ t("logistics.manual.vehiclePlate") }}</span>
    ...
    <section class="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700">
      <h2 class="text-sm font-semibold">{{ t("logistics.manual.datesSection") }}</h2>
      <div class="grid gap-4 sm:grid-cols-2">
        <label class="block">
    ...
    <section class="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700">
      <h2 class="text-sm font-semibold">{{ t("logistics.manual.costSection") }}</h2>
      <div class="grid gap-4 sm:grid-cols-3">
        <label class="block">
          <span class="mb-1 block text-sm font-medium">{{ t("logistics.cost.carrierCost") }}</span>
          <input v-model.number="draft.carrier_cost" type="number" step="0.01" class="form-input" />
        </label>
        <label class="block">
          <span class="mb-1 block text-sm font-medium">{{ t("logistics.cost.customerCharge") }}</span>
          <input v-model.number="draft.customer_charge" type="number" step="0.01" class="form-input" />
        </label>
        <label class="block">
          <span class="mb-1 block text-sm font-medium">{{ t("logistics.cost.paidBy") }} *</span>
    ...
    <p v-if="!isValid" class="text-xs text-slate-500">{{ t("logistics.manual.requiredHint") }}</p>
  </form>
</template>
...
<script setup>
  ...
  watch(() => props.modelValue, (next) => { draft.value = cloneDoc(next); }, { deep: true });
