  ...
  <div class="space-y-4">
    ...
    <div class="card !p-4">
      <div class="flex items-start justify-between gap-3 flex-wrap">
        <div class="min-w-0">
          <p class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.fields.deliveredAt") }}</p>
      ...
      <dl class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <div>
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.fields.receivedBy") }}</dt>
          <dd class="text-[13px] text-gray-900 dark:text-gray-100">
            {{ pod.received_by || "—" }}
            <span v-if="pod.received_by_title" class="text-gray-500 dark:text-gray-400">
              · {{ pod.received_by_title }}
            </span>
          </dd>
        </div>
        <div>
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.fields.deliveredPackages") }}</dt>
        ...
        <div v-if="hasPallets">
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.detail.palletExchange") }}</dt>
          <dd class="text-[13px] text-gray-900 dark:text-gray-100">
            {{ dash(pod.delivered_pallet_count) }} / {{ dash(pod.returned_pallet_count) }}
          </dd>
        </div>
        <div v-if="pod.waybill_number">
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.fields.waybill") }}</dt>
          <dd class="text-[13px] font-mono text-gray-900 dark:text-gray-100">{{ pod.waybill_number }}</dd>
        </div>
        <div v-if="pod.location_source">
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.fields.locationSource") }}</dt>
          <dd class="text-[13px] text-gray-900 dark:text-gray-100">
        ...
        <div v-if="pod.location_recorded_at">
          <dt class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.detail.locationRecordedAt") }}</dt>
    ...
    <div class="card !p-4">
      ...
      <div v-if="!mediaVisible" class="flex items-start gap-2 text-[13px]">
        <AppIcon name="lock" :size="14" class="mt-0.5 shrink-0 text-gray-400 dark:text-gray-500" />
        <div>
          <p class="text-gray-700 dark:text-gray-300">{{ t("logistics.pod.detail.mediaHidden") }}</p>
          <p class="text-xs text-gray-400 dark:text-gray-500">{{ t("logistics.pod.detail.mediaHiddenHint") }}</p>
        </div>
      </div>

      <div v-else class="grid gap-3 sm:grid-cols-3">
        <figure v-for="m in mediaSlots" :key="m.key" class="min-w-0">
          <figcaption class="text-xs text-gray-400 dark:text-gray-500 mb-1">{{ m.label }}</figcaption>
          <div
          ...
          >
            ...
            <p v-if="!m.url" class="text-xs text-gray-400 dark:text-gray-500 px-2 text-center">—</p>
            <p v-else-if="failed[m.key]" class="text-xs text-gray-500 dark:text-gray-400 px-2 text-center">
