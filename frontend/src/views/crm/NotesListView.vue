<template>
  <div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      >
        <div class="flex items-start justify-between mb-2">
          <h4 class="text-[13px] font-bold text-gray-900 dark:text-gray-100 truncate">{{ n.title }}</h4>
          <span class="text-[10px] text-gray-400 shrink-0 ml-2">
            <RelativeTime :value="n.modified" />
          </span>
        </div>
        <p class="text-[12px] text-gray-600 dark:text-gray-300 line-clamp-3 whitespace-pre-wrap">{{ n.content }}</p>
        <div class="flex items-center justify-between mt-3 pt-2 border-t border-gray-100 dark:border-white/5">
          <span class="text-[10px] text-gray-400">
            {{ n.reference_doctype === 'CRM Lead' ? 'Lead' : n.reference_doctype === 'CRM Deal' ? 'Anlaşma' : (n.reference_doctype || 'Genel') }}
          </span>
    ...
    <ListPagination v-if="store.notes.length" v-model="page" :total="store.total" :page-size="pageSize" @update:model-value="load" />
  </div>
</template>
...
function refLink(n) {
  ...
  if (n.reference_doctype === 'CRM Lead') return `/crm/leads/${encodeURIComponent(n.reference_docname)}`
  if (n.reference_doctype === 'CRM Deal') return `/crm/deals/${encodeURIComponent(n.reference_docname)}`
