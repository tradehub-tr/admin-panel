    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="text-xs text-gray-400">İlgili Ürünler bölümünün davranışını ve eşiklerini yönetin</p>
        </div>
      </div>
      <div class="flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        >
          <i :class="rebuilding ? 'fas fa-spinner fa-spin' : 'fas fa-rotate'" class="mr-1.5 text-xs"></i>
      ...
      <div class="card p-4 mb-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="rounded border border-gray-100 bg-gray-50 p-3">
            ...
            <div class="text-base font-bold text-gray-900 mt-0.5">{{ formatNum(status.active_listings) }}</div>
          </div>
          <div class="rounded border border-gray-100 bg-gray-50 p-3">
            <div class="text-[11px] text-gray-500 uppercase tracking-wide">Aktif Kategori</div>
            <div class="text-base font-bold text-gray-900 mt-0.5">{{ formatNum(status.active_categories) }}</div>
          </div>
          <div class="rounded border border-gray-100 bg-gray-50 p-3">
            <div class="text-[11px] text-gray-500 uppercase tracking-wide">Cache Satırı</div>
            <div class="text-base font-bold text-gray-900 mt-0.5">{{ formatNum(status.cache_total) }}</div>
          </div>
          <div class="rounded border border-gray-100 bg-gray-50 p-3">
            <div class="text-[11px] text-gray-500 uppercase tracking-wide">Komşu Eşleşme</div>
            <div class="text-base font-bold text-gray-900 mt-0.5">{{ formatNum(status.neighbour_count) }}</div>
          </div>
        </div>
        <div v-if="status.shadow_in_progress" class="mt-3 rounded border border-amber-200 bg-amber-50 p-2 flex items-center gap-2">
          <i class="fas fa-spinner fa-spin text-amber-600 text-xs"></i>
          <span class="text-xs text-amber-800">Bir rebuild işlemi şu anda arka planda çalışıyor.</span>
        </div>
      </div>

      <!-- Per-tab cache breakdown -->
      <div v-if="hasCacheBreakdown" class="card p-4 mb-4">
        <h3 class="text-xs font-bold text-gray-700 mb-3 uppercase tracking-wide">Sekmeye Göre Önerme</h3>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div v-for="tab in tabBreakdown" :key="tab.key" class="rounded border border-gray-100 bg-white p-3">
      ...
      <div class="card p-4 mb-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldNumber
            ...
            :min="1"
            :max="50" :step="1" help="Her sekme için (Benzer / İkame / Tamamlayıcı / Aksesuar) gösterilecek maksimum ürün sayısı."
          ...
          <FieldNumber
            ...
            :min="0"
            :max="1" :step="0.05" help="Bu skorun altındaki eşleşmeler cache'e yazılmaz (0–1 arası)."
          ...
          <FieldNumber
            ...
            :min="1"
            :max="10" :step="0.1" help="Birlikte alım istatistiği için minimum lift değeri (P(B|A)/P(B))."
          ...
          <FieldNumber
            ...
            :min="0"
            :max="1" :step="0.05" help="Cold-start fallback'te kategori benzerlik eşiği."
          ...
          <FieldNumber
            ...
            :min="0"
            :max="1" :step="0.05" help="Aksesuar adayının ana üründen en fazla bu oranda olması beklenir."
...
async function triggerRebuild() {
  ...
  if (!confirm('Tüm öneri matrisini yeniden hesaplamak istediğinize emin misiniz? Bu işlem birkaç dakika sürebilir ve arka planda işler.')) return
  rebuilding.value = true
  try {
    const res = await api.callMethod('tradehub_core.api.recommendations.trigger_rebuild')
    const msg = res?.message || {}
    alert(`Rebuild tetiklendi: ${msg.chunks || 0} parça (${msg.listings || 0} ürün) long queue'ya gönderildi.`)
  ...
  setup(props, { emit }) {
    return () =>
      h('label', { class: 'flex flex-col gap-1' }, [
        ...
        h('input', {
          type: 'number',
          class: 'rounded border border-gray-200 px-2.5 py-1.5 text-sm focus:border-amber-400 focus:outline-none',
    ...
    return () => {
      ...
      return h('div', { class: 'card p-4' }, [
        h('div', { class: 'flex items-center justify-between mb-3' }, [
          h('h3', { class: 'text-xs font-bold text-gray-700 uppercase tracking-wide' }, props.title),
          h(
            'span',
            {
              class: [
