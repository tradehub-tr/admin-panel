<template>
  ...
  >
    ...
    >
      ...
      <span v-else class="text-xs italic text-slate-400">{{ t("logistics.packing.unsavedPackage") }}</span>
      <span class="ms-auto text-[11px] text-slate-500">{{ typeLabel }}</span>
    </button>
    ...
    <div class="mt-2 flex flex-wrap items-baseline justify-between gap-2 text-xs">
      ...
      <span class="tabular-nums">
        ...
        <b :class="pkg.is_desi_dominant ? 'text-amber-600 dark:text-amber-400' : ''">{{ pkg.desi }}</b>
      </span>
    </div>

    <!-- Desi ağırlıktan büyükse ücreti O belirliyor. Operasyon hangisinin
         belirleyici olduğunu görmeli — fatura ona göre geliyor. -->
    <p v-if="pkg.is_desi_dominant" class="mt-1 text-[11px] text-amber-600 dark:text-amber-400">
    ...
    <div class="mt-2 border-t border-slate-100 pt-2 text-[11px] dark:border-slate-800">
      <p v-if="!contentLines.length" class="text-red-600 dark:text-red-400">
      ...
      <ul v-else class="space-y-0.5">
        <li v-for="line in contentLines" :key="line.rowId" class="flex items-center justify-between gap-2">
          ...
          <span class="flex items-center gap-1.5">
            <span class="tabular-nums text-slate-500">{{ line.qty }} {{ line.uom }}</span>
            <button
              v-if="canWrite"
              type="button"
    ...
    <div v-if="canWrite" class="mt-2 flex flex-wrap gap-2">
      ...
      <button type="button" class="th-btn-outline text-xs" @click="$emit('duplicate')">
        {{ t("logistics.packing.duplicate") }}
      </button>
      <button type="button" class="th-btn-outline text-xs" @click="$emit('remove')">
