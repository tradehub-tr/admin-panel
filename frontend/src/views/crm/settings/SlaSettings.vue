  ...
  <div class="card p-5">
    <div class="flex items-start justify-between mb-5">
      <div>
        ...
        <p class="text-xs text-gray-400">Hizmet seviyesi anlaşmaları ve yanıt/çözüm süre hedefleri.</p>
      </div>
        ...
        <div class="flex items-center justify-between">
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              <h4 class="text-[13px] font-bold text-gray-900 dark:text-gray-100">{{ s.sla_name || s.name }}</h4>
              <span v-if="s.default" class="crm-pill crm-pill-brand">Varsayılan</span>
              <span
                class="crm-pill"
                :class="s.enabled ? 'crm-pill-success' : ''"
              >
                {{ s.enabled ? 'Aktif' : 'Pasif' }}
              </span>
            </div>
            <p v-if="s.description" class="text-[11px] text-gray-500 mt-1 line-clamp-1">{{ s.description }}</p>
          </div>
