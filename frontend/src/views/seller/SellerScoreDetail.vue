          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            <div v-for="cat in scoreCategories" :key="cat.field" class="score-card">
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-sm font-bold" :style="{ color: getScoreColor(doc[cat.field]) }">{{
                  formatScore(doc[cat.field])
                }}</span>
              </div>
              <div class="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  :style="{
                    width: getScorePercent(doc[cat.field]) + '%',
                    background: getScoreColor(doc[cat.field]),
                  }"
                ></div>
              </div>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
            <div class="text-center p-3 bg-gray-50 rounded-xl">
              <p class="text-xl font-black text-gray-800">
                {{ doc.average_rating ? Number(doc.average_rating).toFixed(1) : "-" }}
              </p>
              <div class="flex items-center justify-center gap-0.5 mt-1">
                <i
                  ...
                  class="text-[10px]"
                  :class="
                    s <= Math.round(doc.average_rating || 0)
